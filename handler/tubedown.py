...
def download_url_resource_local(url:str, filename:str, _chunk_size=3*1024)->bool:
    ''' 下载url资源到本地 '''
    ua = get_random_ua()
    ...
    logger.info(f"download_url_resource_local > {url} → {filename}")
    headers = {
        'accept': 'application/json',
        'accept-language': 'zh-CN,zh;q=0.9,en-GB;q=0.8,en;q=0.7,en-US;q=0.6',
        'cache-control': 'no-cache',
    ...
    try:
        ...
        print(f"\ndownload_url_resource_local download succeed. file: {filename}")
        return filename
    except Exception as e:
...
if __name__ == '__main__':
    ...
    download_url_resource_local(video_url, r"./download/test.mp4")